...
# database
conn = pymysql.connect(host='192.168.153.110', port=31802, user='root', password='tmaxfintech', db='financeDB', charset='utf8', autocommit=True, cursorclass=pymysql.cursors.DictCursor)
cur = conn.cursor()

today = datetime.date.today() - datetime.timedelta(days=1)
stamp = int(time.mktime(today.timetuple()))
...
for col in info:
    ...
    symbol = symbols[col]
    df = yf.download(symbol, start=today, end=today)
    df = df.reset_index()
    data = dict(df.iloc[0])
